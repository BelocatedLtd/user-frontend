...
const TaskEarn = () => {
    ...
    const { filteredAdverts } = location.state || {};
    ...
    useEffect(() => {
        ...
        const filteredTasks = filteredAdverts.filter(advert => {
            ...
            const religionMatch = advert.religion === user.religion || advert.religion === 'All';
            const genderMatch = advert.gender === user.gender || advert.gender === 'All'; 
            
            return locationMatch && communityMatch && religionMatch && genderMatch;
    })
    ...
    const handleSelect = async(e, advert_Id) => {
        ...
        const taskCreatedAlreadyByUser = tasks?.find(task => task.taskPerformerId === userId);
        ...
        // if (!taskCreatedAlreadyForUser) {
        //     const response = await dispatch(createNewTask(taskData))
        //     setNewTask(response.payload)
        //      navigate(`/dashboard/submittask/${response?.payload?._id}`)
        // } else if (taskCreatedAlreadyForUser) {
        //     toast.success("You've already applied to perform this task")
        //     navigate(`/dashboard/task/${taskCreatedAlreadyForUser?._id}`)
        ...
        <div>
            <div className='flex items-center gap-3 border-b border-gray-200 pb-6'>
                    ...
                    <div className='flex flex-col'>
                        <p className='font-semibold text-xl text-gray-700'>Perform Social Tasks on {platform} and Earn</p>
        ...
        <div className='px-8 mt-8'>
         {finalFilteredTasks?.map(task => (
            <div className='flex items-center justify-between bg-gray-50 p-6 mb-[2rem]'>
                ...
                <div>
                    <button onClick={e => handleSelect(e, task._id)} className='flex items-center gap-1 bg-secondary text-primary py-2 px-5 rounded-2xl'>
                    {/* <span>{isLoading && <LoaderIcon />}</span>  */}
                        {
                            taskSubmit && "Task Submitted"
                        }
                        {
                            !taskSubmit && "Perform Task"
                        }
                        <span>{isLoading && <LoaderIcon />}</span>
                        </button>
                </div>
