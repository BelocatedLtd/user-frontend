...
const WithdrawalRequests = () => {
	...
	const columns = [
		...
		},
		{
			...
			button: true,
			cell: (row) => {
				const userWithdrawalRequest =
					withdrawalList?.find((wdrequest) => wdrequest?._id === row?._id) || {}
				setWdData(userWithdrawalRequest)
				return (
					<button
						className='bg-[#18141E] text-gray-100 px-6 py-2 rounded-2xl hover:bg-btn hover:bg-secondary'
	...
	const handleButtonClick = (e, withdrawalRequestId) => {
		...
		navigate(`/admin/dashboard/withdrawals/confirm/${withdrawalRequestId}`, {
