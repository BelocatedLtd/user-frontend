...
const Wallet = ({ handleEarn, handleAdvertise }) => {
	...
	return (
		<div className='w-full h-full flex flex-col justify-between items-center border border-gray-200 rounded-2xl py-6'>
			{selectFundingBtn && (
				<FundingForm toggleFundingSelect={toggleFundingSelect} />
			)}
			{togleWithdrawBtn && (
				<WithdrawalForm
			...
			<div className='mt-[1.5rem]'>
				<h1 className='text-3xl text-gray-800 font-extrabold text-center'>
					{isLoading ? <LoaderIcon /> : <span>₦{wallet?.value}</span>}
				</h1>
        ...
        (<div className='w-full flex items-center gap-2 text-sm text-gray-400 font-extrabold text-center'>
          ...
          {user?.referralBonusPts >= 50 ? (<button onClick={convertPts} className='bg-secondary text-primary text-[8px] px-1 rounded-2xl'>Convert</button>) : ""}
			...
			<div className='hidden md:flex gap-2 mt-[1.5rem]'>
				<button
					onClick={toggleFundingSelect}
					className='flex-1 bg-secondary text-[9px] md:text-[12px] text-gray-100  px-7 py-0 md:py-3 rounded-full hover:bg-transparent hover:text-tertiary hover:border-tertiary hover:border md:px-10'>
					Fund
				</button>
				<button
					onClick={handleWithdrawFunds}
					className='flex-1 bg-transparent border border-gray-500 text-[12px] text-gray-600 px-4 py-2 md:py-3 rounded-full hover:bg-transparent hover:text-tertiary hover:border-tertiary hover:border'>
					Withdraw
				</button>
			</div>
			...
			<div className='flex items-center justify-center gap-[2rem] mt-3 md:hidden'>
				<button
					onClick={handleEarn}
					className='flex-1 bg-secondary text-[12px] text-gray-100  px-7 py-2 md:py-3 rounded-full hover:bg-transparent hover:text-tertiary hover:border-tertiary hover:border md:px-10'>
					Earn
				</button>
			...
			<div className='md:hidden flex gap-2 mt-[1.5rem]'>
				<button
					onClick={toggleFundingSelect}
					className='flex items-center gap-1 text-sm text-green-600 mr-[2rem]'>
					<HiOutlineArrowLeft /> Fund
				</button>
				<button
					onClick={handleWithdrawFunds}
			...
			<div className='w-[70%] flex flex-col items-center gap-2 mt-[2rem]'>
				<button
					onClick={handleRefBonus}
					className='w-full flex items-center gap-1 justify-center bg-tertiary rounded-full text-primary py-2'>
					Referral Bonus
				</button>
				<button
					onClick={handleRefChallenge}
					className='w-full bg-secondary rounded-full text-primary py-2'>
					Referral Challenge
				</button>
			</div>
			...
			<div className='flex px-6 items-center justify-center mt-[4rem] pb-4 gap-5 md:flex'>
				<div className='flex flex-col items-center justify-center mb-[1rem] md:mb-[0rem]'>
					<h3 className='font-semibold text-gray-600 text-[9px] md:text-[12px] text-center'>
						Total Earnings
					</h3>
					<small>
						{isLoading ? <LoaderIcon /> : <span>₦{wallet?.totalEarning}</span>}
					</small>
				</div>
				<div className='flex flex-col items-center justify-center mb-[1rem] md:mb-[0rem]'>
					<h3 className='font-semibold text-gray-600 text-[9px] md:text-[12px] text-center'>
					...
					<small>
						{isLoading ? (
							...
							<span>₦{wallet?.pendingBalance}</span>
						)}
					</small>
				</div>
				<div className='flex flex-col items-center justify-center mb-[1rem] md:mb-[0rem]'>
					...
					<small>
						{isLoading ? <LoaderIcon /> : <span>₦{wallet?.amountSpent}</span>}
					</small>
				</div>
			</div>
		</div>
