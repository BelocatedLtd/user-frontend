  ...
  return (
    <div className='widget flex flex-1 justify-between p-[10px] shadow-lg rounded-[10px] h-[150px]'>
        ...
        <div className="right flex flex-col justify-between">
            <div className={`percentage flex items-center text-[14px] ${percentage >= 0 && ('text-green-500') } ${percentage <= 0 && ('text-red-500') }`}>
                {percentage >= 0 && <IoIosArrowUp />}
                {percentage <= 0 && <IoIosArrowDown />}
                {percentage}
            </div>
            {data?.icon}
        </div>
