			...
			<div className='w-full flex justify-center'>
				<div className='w-[80%] h-fit py-[2rem] px-[2rem] mt-[1rem] bg-primary border border-blue-300 rounded-2xl shadow-xl'>
					<div className='w-full flex flex-col items-center'>
						<label className='font-bold text-gray-600 text-center'>
							Point Balance
						</label>
						<div className='flex justify-center'>
							<h1 className='text-[45px] text-gray-800 font-extrabold text-center'>
								{progress}
							</h1>
							<small className='text-center flex items-center mt-5 text-[8px] text-gray-400'>
								1 Point = ₦50
							</small>
						</div>
						<div className='w-full flex flex-col items-center mb-[2rem]'>
							...
							<p className=''>
								{progress >= 50
									? 'You have crossed the threshold, redeem your points'
									: progress >= 1
									? `You need ${50 - progress} more points to redeem`
									: 'You need atleast 50 points to redeem'}
							</p>
						</div>

						<div className='w-full flex justify-center'>
							{user?.referralBonusPts >= 50 ? (
								<button
									onClick={convertPts}
									className='bg-red-600 text-primary px-4 py-2 rounded-2xl'>
									Redeem Points
								</button>
							) : (
								''
			...
			<div className='w-full flex flex-col items-center gap-2 justify-center mt-[1rem]  md:gap-0'>
				...
				<div className='flex items-center justify-center gap-2  w-[60%]'>
					<input
						...
						readOnly
						ref={inputRef}
						className='w-fit md:w-1/2 p-3 border border-gray-200 rounded-lg items-center'
					/>
					<Image
						src={copy}
