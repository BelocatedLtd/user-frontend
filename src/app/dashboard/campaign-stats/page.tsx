...
export default CampaignStatsCampaignStats */    
'use client'
import Button from '@/components/Button'
import PaginatedComponent from '@/components/Pagination'
import { getUserAdverts } from '@/services/advertService'
...
const CampaignStats = () => {
	...
	return (
		<div className='w-full p-5 md:p-10'>
			...
			{/* Header Section */}
			<div className='flex flex-col md:flex-row items-center justify-between gap-3 py-5 border-b border-gray-200'>
				<div className='flex flex-col'>
					<p className='font-semibold text-xl text-gray-700'>
						My Ad Campaigns
					</p>
					<small className='font-medium text-gray-500'>
						Click <span className='text-secondary cursor-pointer'>here</span> to
						see and monitor your adverts
					</small>
				</div>
				<div className='flex items-center gap-5 mt-3 md:mt-0'>
					...
					<Button
						...
						color='danger'
						onClick={() => router.push('/dashboard/advertise')}
						className='flex items-center gap-1 bg-secondary text-primary rounded-full px-5 py-2 text-[12px] md:text-[15px] hover:bg-tertiary'
					>
						<BsFillPlusCircleFill />
						Campaign
					</Button>
