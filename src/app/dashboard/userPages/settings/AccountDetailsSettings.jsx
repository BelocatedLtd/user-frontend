...
const AccountDetailsSettings = ({ user }) => {
	...
	const handleSubmit = async (e) => {
		...
		if (
			accountDetails?.phone == user.phone &&
			...
			if (
				accountDetails.phone.length > 11 ||
				accountDetails.phone.length < 10
			) {
			...
			const updatedUserDetails = await updateUserAccountDetails(
				accountDetailsData,
			)
				...
				<div className='flex flex-col mt-6 gap-6'>
					<div className=''>
						...
						<input
							...
							disabled
							className='w-full shadow-inner p-3 border 
                  border-gray-200 rounded-xl text-gray-400'
						/>
					</div>

					<div className=''>
						<label htmlFor='Email' className='text-left mb-1 ml-1'>
							Email
						</label>
						<input
							type='email'
					...
					<div className=''>
						<label htmlFor='Phone' className='text-left mb-1 ml-1'>
							Phone Number
						</label>
						<input
							type='number'
							...
							onChange={handleAccountDetailsInputChange}
							className='w-full shadow-inner p-3 rounded-xl'
						/>
